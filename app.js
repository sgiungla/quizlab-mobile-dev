...
async function resetPerformance(){
 ...
 const ok=confirm('Azzero tutte le performance di questa materia?\n\nVerranno cancellati tentativi, simulazioni, statistiche, errori e percorso Materia completa.\nLa banca domande e le domande segnate resteranno intatte.');
 if(!ok)return;
 const really=confirm('Conferma definitiva: questa operazione azzera anche i dati sincronizzati nel cloud per questa materia.');
 if(!really)return;
 c.attempts=[];
 c.exams=[];
 c.pendingReview=[];
